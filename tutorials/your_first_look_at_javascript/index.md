...
{{Byline}}
{{Summary_Section|Now it is time to get your hands dirty. This article provides a basic introduction to coding with JavaScript.}}
{{Tutorial
|Content=== Introduction ==
 
In this article of the [http://www.w3.org/wiki/Web_Standards_Curriculum Web Standards Curriculum], we will cover the basics of JavaScript — how and where to use it, what problems to avoid, and general basics to get you started on your journey towards becoming a top-notch JavaScript developer.
...
JavaScript does not have to stay inside browsers. To run JavaScript in console environment, please check out [https://developer.mozilla.org/en-US/docs/Rhino/Download_Rhino Mozilla Rhino]; to run JavaScript in server environment, please check [http://nodejs.org/ node.js].
...
* <code>&lt;noscript&gt;&lt;/noscript&gt;</code> — as its name implies, the <code>noscript</code> element is the opposite to the <code>script</code> element. The content inside it will be shown to users that have JavaScript disabled. The main reason to use <code>noscript</code> is to provide alternative content for users who do not have JavaScript available in their browser. Users that have no JavaScript do not do that to make your life harder — they might do it because of a security policy, or because the browser they are using does not support JavaScript. But what you can provide inside <code>noscript</code> is limited, and you should not use it provide a note telling users to enable JavaScript in their browser — for some users this is not possible.
