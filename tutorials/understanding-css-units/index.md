...
{{!}}Useful to size boxes that adapt to different viewport heights. For example, may be used to set a maximum height on an image so that it does not exceed the viewport dimensions.
{{!}}-
{{!}}vmin
...
# Use [[svg|SVG]] (or [http://css-tricks.com/examples/IconFont/ icon fonts], more limited but more widely supported) wherever you can (depending on the image type and/or your target browsers) to have content that naturally scales up to higher pixel densities or sizes.
