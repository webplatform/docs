...
HTML5 defines a <code>&lt;nav&gt;</code> element, which is to be used to contain the primary navigation of a website, be it a list of links or a form element such as a search box. This is a good idea. Previously, we would have contained the navigation block inside something like <code>&lt;div id="navigation"&gt;</code>. While this might work pretty well for styling purposes, it is a <code>&lt;div&gt;</code>, and therefore semantically anonymous. <code>&lt;nav&gt;</code> provides a consistent way to unambiguously define the primary navigation areas. This is good for search engine optimization and for visually impaired users. A user of a screen reader can more easily find the navigation area if it is clearly signposted (this does depend on the screen reader they are using supporting the <code>&lt;nav&gt;</code> element, so it might be a little way off yet). A navigation block would look something like this:
...
If you haven’t read the [http://www.w3.org/wiki/HTML_links_-_lets_build_a_web links] and [http://www.w3.org/wiki/HTML_lists lists] articles earlier in the course, you should.  They are required prerequisites for understanding this one.
