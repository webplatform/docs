...
|Description=This example uses the '''input type{{=}}number''' element to create an empty number control.
|Code=&lt;input type{{=}}"number" value{{=}}"" name{{=}}"numbox"&gt;
}}
}}
{{Notes_Section
|Notes====Remarks===
The [[html/attributes/size (control)|'''size''']] and [[html/attributes/maxLength|'''maxlength''']] attributes are considered obsolete, but conform under the [http://go.microsoft.com/fwlink/?LinkID{{=}}239925 HTML5 specification].
...
*[http://go.microsoft.com/fwlink/p/?linkid{{=}}221374 HTML5 A vocabulary and associated APIs for HTML and XHTML], Section 4.10.7.1.13
