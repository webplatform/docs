...
The [[css/properties/height|'''height''']] and [[css/properties/width|'''width''']] styles are exposed to the '''input type{{=}}checkbox''' element. The size of the element is set based on the values provided by the author, subject to a particular minimum. The size is calculated as follows:
*If the [[css/properties/height|'''height''']] or [[css/properties/width|'''width''']] is greater than 20 pixels, the padding around the check box is set to 4 pixels, and the inner height or width is set to 8 pixels.
*If the [[css/properties/height|'''height''']] or [[css/properties/width|'''width''']] is less than 20 pixels but greater than 13 pixels, the padding around the check box is equal to one half the specified '''height''' or '''width''' minus 13. For example, if the specified '''width''' of the check box is 17, the equation would be: (17-13)/2.
*If the [[css/properties/height|'''height''']] or [[css/properties/width|'''width''']] is less than 12 pixels, the padding around the check box is set to 0 and the inner width is set to the value specified by the author.
...
In IE8 Standards mode and later, inserting an '''input type{{=}}checkbox''' object into the Document Object Model (DOM) tree does not reset the [[dom/properties/defaultChecked|'''defaultChecked''']] property. For more information, see Defining Document Compatibility.
...
|-
|[[dom/properties/defaultChecked|'''defaultChecked''']]
|Sets or retrieves the state of the check box or radio button.
|-
...
|-
|[[dom/properties/indeterminate|'''indeterminate''']]
|Sets or retrieves whether the user has changed the status of a '''input type{{=}}checkbox''' element. On a [[html/elements/progress|'''HTMLProgressElement''']] element will show activity but not a specific amount.
|-
