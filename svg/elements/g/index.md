...
|Content=The '''g''' element (&lt;g&gt;) groups two or more graphics elements or other '''g''' elements together to provide document structure. Groups containing the '''[http://docs.webplatform.org/wiki/svg/elements/desc desc]''' and '''[http://docs.webplatform.org/wiki/svg/elements/title title]''' elements, provide additional semantic information and increase accessibility. When defined inside of a '''[http://docs.webplatform.org/wiki/svg/elements/defs defs]''' element and named using the '''id''' attribute, groups can be reused.
}}
{{Examples_Section
|Not_required=No
