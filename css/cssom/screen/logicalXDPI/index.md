...
{{API_Name}}
{{Summary_Section|Retrieves the screen's horizontal Dots Per Inch (DPI) value.}}
{{API_Object_Property
...
|Language=JavaScript
|Description=The following examples use the '''logicalXDPI''' property to retrieve the normal horizontal DPI of the screen. The function in this example returns <code>1</code> if Internet Explorer is not adjusting the scale of the screen.
|Code=&lt;script&gt;
