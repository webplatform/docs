...
{{API_Name}}
{{Summary_Section|A grid item's placement in a grid area consists of a grid position and a grid span. The grid-column-end property (with [[css/properties/grid-row-start|grid-row-start]], [[css/properties/grid-row-end|grid-row-end]], and [[css/properties/grid-column-start|grid-column-start]]) determines a grid item's placement by specifying the grid lines of a grid item's grid area.}}
{{CSS Property
|Initial value=auto
