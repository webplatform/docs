...
{{CSS_Property_Value|Data Type=percentage |Description=Integer followed by a percent sign (%) that specifies a percentage of the containing block height to use as the minimum height of the element. If the height of the containing block is not explicitly set, then the element has no minimum height and the  '''min-height''' property is interpreted as 0%. For Internet Explorer 6, information on containing blocks and how the height is computed, see the [http://go.microsoft.com/fwlink/p/?linkid{{=}}203757 Cascading Style Sheets, Level 2 (CSS2)] specification. For Internet Explorer 7, see the [http://go.microsoft.com/fwlink/p/?linkid{{=}}203760 Cascading Style Sheets, Level 2.1 (CSS2.1)] specification.}}
}}
{{Topics|CSS}}
{{Examples_Section
|Not_required=No
