{{Page_Title|margin-left}}
{{Flags
|Checked_Out=No
}}
{{Standardization_Status|W3C Recommendation}}
{{API_Name}}
{{Summary_Section|<code>margin-left</code> sets the left margin of an element.}}
{{CSS Property
...
|Animatable=No
|CSS object model property=marginLeft
...
|Data Type=auto
|Description=The browser calculates a left margin dependent on the space available.
}}{{CSS Property Value
|Data Type=inherit
|Description=Inherits the parent element's specified <code>margin-left</code> width.
}}
}}
{{Examples_Section
|Not_required=No
...
|Language=HTML
|Description=In this example there are three blocks, styled identically except for their <code>margin-left</code> values:

* The first one has a <code>margin-left</code> of 2 centimeters, meaning that it is pushed over to the right by 2cm.
* The second one has no <code>margin-left</code> of its own, to indicate the default position of the blocks.
* The bottom block has a <code>margin-left</code> of -10% set on it, meaning that it is pushed over to the left by 10% of the parent element's width.
|Code=&lt;div class="one"&gt;&lt;/div&gt;
&lt;div class="two"&gt;&lt;/div&gt;
&lt;div class="three"&gt;&lt;/div&gt;
|LiveURL=http://code.webplatform.org/gist/5727939
}}{{Single Example
|Language=CSS
