...
<p>Note that there are three parts of a CSS rule. First, we use the selector to select a specific element (<pre>h1</pre>). We then specify arguments. The properties, or attributes are needed to tell the browser what exactly is being manipulated (<pre>color</pre>) and finally the value of that property is specified (<pre>blue</pre>). As with other programming languages, the arguments part is placed within curly braces (<pre>{these}</pre>) and each argument is terminated by a semi-colon (<pre>;</pre>).
