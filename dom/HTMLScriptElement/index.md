...
[text](/dom/HTMLScriptElement/text)
:   Legacy. Use [textContent](/dom/Node/textContent) instead. When setting, does same as [textContent](/dom/Node/textContent). When getting, gets a concatenated version of all of the child text nodes of a [script](/html/elements/script) element.
...
:   Use the [type](/html/attributes/type_(script_element)) attribute instead. Specifies the language of the script to be evaluated. May be omitted when using ECMAScript (also known as JavaScript).
