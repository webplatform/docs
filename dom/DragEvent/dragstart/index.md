...
|Description=This example uses event bubbling to handle the '''ondragstart''' event at the '''body''' level.
|Code=&lt;body ondragstart{{=}}"console.log(event.srcElement.tagName)"&gt;
...
&lt;/body&gt;
|LiveURL=http://samples.msdn.microsoft.com/workshop/samples/author/dhtml/refs/ondragstartEX.htm
}}{{Single Example
...
{{Notes_Section
|Notes====Remarks===
The '''ondragstart''' event is the first to fire when the user starts to drag the mouse. It is essential to every drag operation, yet is just one of several source events in the data transfer object model. Source events use the [[dom/DataTransfer/setData|'''setData''']] method of the [[dom/DataTransfer|'''dataTransfer''']] object to provide information about data being transferred. Source events include '''dragstart''', [[dom/DragEvent/drag|'''drag''']], and [[dom/DragEvent/dragend|'''dragend''']].
