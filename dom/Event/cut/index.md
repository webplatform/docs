...
#Set '''event.returnValue{{=}}false''' in the [[dom/events/beforecut|'''onbeforecut''']] event to enable the Cut shortcut menu item.
...
#Invoke the [[dom/methods/setData|'''setData''']] method in the '''oncut''' event.

Set '''event.returnValue{{=}}false''' in the '''oncut''' event handler to cancel the default action. The default action must be canceled to successfully use the [[dom/methods/setData|'''setData''']] method. Web authors can use the [[dom/properties/innerdom/innerHTML|'''innerHTML''']] property or the [[dom/traversal/methods/createRange (selection)|'''createRange''']] method to perform the cut operation once the event is canceled.
