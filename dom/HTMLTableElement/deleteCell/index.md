...
{{API_Name}}
{{API_Object_Method
|Parameters={{Method Parameter|Name=index|Data type=Integer|Description='''Integer''' that specifies the zero-based position of the cell to remove from the table row. If no value is provided, the last cell in the [[dom/properties/cellSpacing|'''cells''']] collection is deleted.|Optional=}}
