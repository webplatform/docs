...
{{API_Name}}
{{Summary_Section|Returns the time immediately before the user agent starts establishing the connection to the server to retrieve the resource.}}
{{API_Object_Property
...
{{Notes_Section
|Notes=If a page is retrieved from an application cache, '''connectStart''' has the same value as '''fetchStart'''.
...
|URL=http://www.w3.org/TR/resource-timing/#dom-performanceresourcetiming-connectstart
